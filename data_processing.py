"""
Data processing utilities for the Two Towers CEO-Firm matching model.

This module contains:
- DataProcessor: Handles loading, cleaning, feature engineering, encoding, and scaling
- CEOFirmDataset: PyTorch Dataset for mini-batch training
"""
import pandas as pd
import numpy as np
import torch
...
class DataProcessor:
    ...
    Encapsulates the state (scalers/encoders) for consistency between 
    training and inference.
    
    Attributes:
        cfg: Config instance with feature definitions
        encoders: Dictionary of LabelEncoders for categorical features
        scalers: Dictionary of StandardScalers for numeric features
        processed_df: Last processed DataFrame (for visualization)
    """
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with required columns, or empty DataFrame on error
        """
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Creates:
        - tenure: fiscalyear - ceo_year (clipped to >= 0)
        - weights: 1 / (sd_match_means^2 + epsilon)
    ...
    def fit(self, df: pd.DataFrame):
        ...
        Args:
            df: Training DataFrame (already cleaned via prepare_features)
        """
    ...
    def transform(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        Args:
            df: DataFrame to transform (must have required columns)
            
        Returns:
            Dictionary with tensors and metadata for model input
        """
        if df.empty:
            return {}
        ...
        # Categorical Encoding (Handle unknown labels safely)
    ...
    def _to_tensors(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with tensors and counts for model initialization
        """
...
class CEOFirmDataset(Dataset):
    ...
    Args:
        data_dict: Dictionary from DataProcessor.transform() containing tensors
    """
